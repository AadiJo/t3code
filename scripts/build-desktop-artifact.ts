...
export class WslNodePtyPrebuildStagingError extends Schema.TaggedErrorClass<WslNodePtyPrebuildStagingError>()(
  "WslNodePtyPrebuildStagingError",
  {
  ...
  override get message(): string {
    return `Could not stage WSL node-pty prebuild ${this.prebuildPath}.`;
  }
}
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  const prebuildPath = input.prebuildPath;
  ...
  const prebuildExists = yield* fs.exists(prebuildPath).pipe(Effect.orElseSucceed(() => false));
  if (!prebuildExists) {
    return yield* new WslNodePtyPrebuildStagingError({
      prebuildPath,
  ...
  const manifest = yield* decodeNodePtyManifest(pkgRaw).pipe(
    Effect.mapError(
      (cause) =>
        new WslNodePtyPrebuildStagingError({
          prebuildPath,
          stageAppDir: input.stageAppDir,
          cause,
        }),
  ...
  yield* fs.copyFile(prebuildPath, path.join(prebuildDir, "pty.node"));
